...
const sync = async (datasetName) => {
  ...
  for (const item of items) {
    ...
    for (const hour of item.hours) {
      item.openHours[hour.day] = {};
      item.openHours[hour.day].start = {};
      item.openHours[hour.day].end = {};
      item.openHours[hour.day].start['1'] = hour.startTime;
      item.openHours[hour.day].end['1'] = hour.endTime;
    }

    // eslint-disable-next-line no-plusplus
    for (let i = 0; i <= item.facility_names.length; i++) {
      if (item.facility_names.some((name) => name !== '') && typeof item.facility_names[i] !== 'undefined') {
...
Apify.main(async () => {
  const resResult = await sync('tripadvisor.com.vn-restaurants');
